...
//双循环链表类的实现
template<class T>
DLinkList<T>::DLinkList()//构建函数，建一空双循环链表
{
...
template<class T>
DLinkList<T>::~DLinkList()//析构函数，释放链表所占空间
{
	Node<T> *p,*q;
...
template<class T>
void DLinkList<T>::CreateList(int n)
...
template<class T>
void DLinkList<T>::Insert (int i,T e)
{//在指定位置插入元素
